...
import { ChevronDown, ChevronRight, Search, LogOut, BookOpen, Globe, Flame } from 'lucide-react';
...
export default function Sidebar({ isOpen, books, activeBook, activeChapter, onSelectBook, onSelectChapter, onSearchClick, onEbooksOpen, onSimpsonOpen, onBensonOpen, onGillOpen, onClarkeOpen, onMHOpen, onDevotionalOpen }: SidebarProps) {
  ...
  return (
    ...
    >
      <div className={cn("flex-1 overflow-y-auto py-3 custom-scrollbar w-[85vw] sm:w-[240px]", !isOpen && "hidden")}>
        ...
        <div className="mb-1 px-2">
          <button
            onClick={() => onGillOpen && onGillOpen()}
            className="w-full flex items-center justify-between text-[13px] font-medium text-sleek-text-main px-3 py-2 rounded-lg hover:bg-sleek-hover transition-all group"
          >
            <span className="flex items-center gap-2.5">
              <span className="w-6 h-6 rounded-md bg-blue-100 text-blue-600 flex items-center justify-center flex-shrink-0">
                <BookOpen size={13} />
              </span>
              John Gill
            </span>
        ...
        <div className="mb-1 px-2">
          <button
            onClick={() => onClarkeOpen && onClarkeOpen()}
            className="w-full flex items-center justify-between text-[13px] font-medium text-sleek-text-main px-3 py-2 rounded-lg hover:bg-sleek-hover transition-all group"
          >
            <span className="flex items-center gap-2.5">
