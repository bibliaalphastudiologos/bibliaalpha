...
import { X, ChevronLeft, ChevronRight, Loader2, Flame, ChevronDown, ChevronUp } from 'lucide-react';
import { cn } from '../App';
import { getSimpsonNotes, SimpsonNote } from '../services/simpsonApi';
    ...
    <div className="border border-sleek-border rounded-xl overflow-hidden">
      <button
        ...
        className={cn(
          ...
          expanded ? 'bg-amber-50 border-b border-amber-100' : 'hover:bg-sleek-hover'
        )}
      >
        <div className="flex items-start gap-2.5 min-w-0">
          <Flame size={13} className={cn('mt-0.5 shrink-0', expanded ? 'text-amber-500' : 'text-sleek-text-muted')} />
          <div className="min-w-0">
            {note.keywordPt ? (
              <span className={cn('text-[13px] font-semibold leading-snug', expanded ? 'text-amber-800' : 'text-sleek-text-main')}>
                {note.keywordPt}
              </span>
            ) : null}
        ...
        <div className="px-4 py-4 bg-white space-y-3">
          ...
          {note.text && note.text !== note.textPt && (
            <details className="group">
              <summary className="text-[10px] text-amber-500 cursor-pointer select-none list-none flex items-center gap-1 hover:text-amber-700">
...
export default function ABSimpsonPanel({
  ...
  useEffect(() => {
    ...
    let alive = true;
    setLoading(true);
    getSimpsonNotes(bookId, chapter)
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-sleek-bg">
      ...
      <header className="h-12 flex items-center justify-between px-4 sm:px-8 border-b border-sleek-border bg-sleek-bg shrink-0">
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="text-[13px] sm:text-[14px] font-semibold text-sleek-text-main leading-none">A.B. Simpson</h1>
            <p className="text-[10px] text-sleek-text-muted mt-0.5 leading-none truncate">
              Dias do Céu na Terra, 1897 · referências clicáveis
            </p>
          </div>
        </div>
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-8 py-6">
          {loading && (
            <div className="flex flex-col items-center justify-center py-28 gap-3">
              ...
              <p className="text-[12px] text-sleek-text-muted">Carregando e traduzindo meditações…</p>
            </div>
          )}
          {!loading && error && (
            <div className="flex flex-col items-center justify-center py-28 gap-3 text-center">
              <Flame size={28} className="text-sleek-text-muted opacity-30" />
          ...
          {!loading && !error && notes.length === 0 && (
            <div className="flex flex-col items-center justify-center py-28 gap-3 text-center">
              <Flame size={28} className="text-sleek-text-muted opacity-30" />
              <p className="text-[13px] text-sleek-text-muted">Sem meditações de A.B. Simpson para {bookName} {chapter}.</p>
            ...
            <>
              <div className="mb-5">
                <span className="text-[11px] font-semibold uppercase tracking-[0.08em] text-sleek-text-muted">
                  {notes.length} meditação{notes.length !== 1 ? 'ões' : ''} · {bookName} {chapter}
                </span>
              </div>
