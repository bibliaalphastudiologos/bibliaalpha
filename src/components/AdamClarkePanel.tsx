...
import { cn } from '../App';
import { getClarkeNotes, ClarkeNote } from '../services/adamClarkeApi';
...
function parseReferences(text: string): TextSegment[] {
  ...
  while ((match = REF_REGEX.exec(text)) !== null) {
    ...
    segments.push(bookId
      ? { text: match[0], ref: { bookId, chapter: parseInt(match[2], 10), verse: parseInt(match[3], 10) } }
      : { text: match[0] }
    );
      ...
      {segments.map((seg, i) =>
        seg.ref ? (
          <button key={i} onClick={() => onNavigate?.(seg.ref!.bookId, seg.ref!.chapter)}
            className="inline text-emerald-600 hover:text-emerald-800 underline underline-offset-2 decoration-dotted font-medium transition-colors cursor-pointer">
            {seg.text}
          </button>
    ...
    <div className="border border-sleek-border rounded-xl overflow-hidden">
      <button onClick={() => setExpanded(!expanded)}
        ...
        <div className="flex items-center gap-2.5 min-w-0">
          <BookOpen size={13} className={cn('shrink-0', expanded ? 'text-emerald-500' : 'text-sleek-text-muted')} />
          <span className={cn('text-[13px] font-semibold', expanded ? 'text-emerald-700' : 'text-sleek-text-main')}>
            Versículo {note.verseNumber}
          </span>
        </div>
        {expanded ? <ChevronUp size={13} className="text-emerald-400 shrink-0" /> : <ChevronDown size={13} className="text-sleek-text-muted shrink-0" />}
      </button>
      {expanded && (
...
interface AdamClarkePanelProps {
  isOpen: boolean;
  onClose: () => void;
  bookId: string;
  bookName: string;
...
export default function AdamClarkePanel({
  isOpen, onClose, bookId, bookName, chapter, totalChapters,
  onPrevChapter, onNextChapter, onNavigate,
}: AdamClarkePanelProps) {
  const [notes, setNotes]     = useState<ClarkeNote[]>([]);
        ...
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] text-sleek-text-muted mt-0.5 leading-none truncate">
              Adam Clarke, 1810–26 · comentário bíblico completo · referências clicáveis
            </p>
          </div>
        </div>
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-8 py-6">
          {loading && (
            <div className="flex flex-col items-center justify-center py-28 gap-3">
              ...
              <p className="text-[12px] text-sleek-text-muted">Carregando e traduzindo comentário de Adam Clarke…</p>
            </div>
          )}
          {!loading && error && (
          ...
          {!loading && !error && notes.length === 0 && (
            <div className="flex flex-col items-center justify-center py-28 gap-3 text-center">
              <BookOpen size={28} className="text-sleek-text-muted opacity-30" />
              <p className="text-[13px] text-sleek-text-muted">Sem notas de Adam Clarke para {bookName} {chapter}.</p>
