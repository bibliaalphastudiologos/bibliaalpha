...
import { cn } from '../App';
import { getGillNotes, GillNote } from '../services/johnGillApi';
    ...
    <div className="border border-sleek-border rounded-xl overflow-hidden">
      <button onClick={() => setExpanded(!expanded)}
        className={cn('w-full flex items-center justify-between gap-3 px-4 py-3 text-left transition-colors',
          expanded ? 'bg-blue-50 border-b border-blue-100' : 'hover:bg-sleek-hover')}>
        <div className="flex items-center gap-2.5 min-w-0">
          <BookOpen size={13} className={cn('shrink-0', expanded ? 'text-blue-500' : 'text-sleek-text-muted')} />
          <span className={cn('text-[13px] font-semibold', expanded ? 'text-blue-700' : 'text-sleek-text-main')}>
            Versículo {note.verseNumber}
          </span>
        </div>
        {expanded ? <ChevronUp size={13} className="text-blue-400 shrink-0" /> : <ChevronDown size={13} className="text-sleek-text-muted shrink-0" />}
      </button>
      {expanded && (
        <div className="px-4 py-4 bg-white space-y-3">
          ...
          {note.text && note.text !== note.textPt && (
            <details className="group">
              <summary className="text-[10px] text-blue-400 cursor-pointer select-none list-none flex items-center gap-1 hover:text-blue-600">
        ...
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] text-sleek-text-muted mt-0.5 leading-none truncate">
              John Gill, 1748–63 · comentário exegético completo · referências clicáveis
            </p>
          </div>
        </div>
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-8 py-6">
          {loading && (
            <div className="flex flex-col items-center justify-center py-28 gap-3">
              ...
              <p className="text-[12px] text-sleek-text-muted">Carregando e traduzindo comentário de John Gill…</p>
            </div>
          )}
          {!loading && error && (
          ...
          {!loading && !error && notes.length === 0 && (
            <div className="flex flex-col items-center justify-center py-28 gap-3 text-center">
              <BookOpen size={28} className="text-sleek-text-muted opacity-30" />
              <p className="text-[13px] text-sleek-text-muted">Sem notas de John Gill para {bookName} {chapter}.</p>
