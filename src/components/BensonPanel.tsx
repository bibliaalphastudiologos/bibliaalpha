...
import { cn } from '../App';
import { getBensonNotes, BensonNote } from '../services/bensonApi';

// ── Bible reference parser (shared pattern) ───────────────────────────────────
...
const BOOK_NAME_MAP: Record<string, string> = {
  ...
  'Josué':'JOS','Juízes':'JDG','Rute':'RUT','1 Reis':'1KI','2 Reis':'2KI',
  '1 Crônicas':'1CH','2 Crônicas':'2CH','Esdras':'EZR','Neemias':'NEH','Ester':'EST',
  'Jó':'JOB','Salmos':'PSA','Salmo':'PSA','Provérbios':'PRO','Eclesiastes':'ECC',
  'Cântico dos Cânticos':'SNG','Isaías':'ISA','Jeremias':'JER','Lamentações':'LAM',
  'Ezequiel':'EZK','Daniel':'DAN','Oseias':'HOS','Joel':'JOL','Amós':'AMO',
  'Obadias':'OBA','Jonas':'JON','Miquéias':'MIC','Naum':'NAH','Habacuque':'HAB',
...
function parseReferences(text: string): TextSegment[] {
  ...
  while ((match = REF_REGEX.exec(text)) !== null) {
    ...
    segments.push(bookId
      ? { text: match[0], ref: { bookId, chapter, verse } }
      : { text: match[0] }
    );
      ...
      {segments.map((seg, i) =>
        seg.ref ? (
          <button
            ...
            className="inline text-violet-600 hover:text-violet-800 underline underline-offset-2 decoration-dotted font-medium transition-colors cursor-pointer"
    ...
    <div className="border border-sleek-border rounded-xl overflow-hidden">
      <button
        ...
        className={cn(
          ...
          expanded ? "bg-violet-50 border-b border-violet-100" : "hover:bg-sleek-hover"
        )}
      >
        <div className="flex items-start gap-2.5 min-w-0">
          <BookOpen size={13} className={cn("mt-0.5 shrink-0", expanded ? "text-violet-500" : "text-sleek-text-muted")} />
          <div className="min-w-0">
            {note.keywordPt ? (
              <span className={cn("text-[13px] font-semibold leading-snug", expanded ? "text-violet-700" : "text-sleek-text-main")}>
                {note.keywordPt}
              </span>
            ) : null}
        ...
        {expanded
          ? <ChevronUp size={13} className="text-violet-400 shrink-0" />
        ...
        <div className="px-4 py-4 bg-white space-y-3">
          ...
          {note.text && note.text !== note.textPt && (
            <details className="group">
              <summary className="text-[10px] text-violet-400 cursor-pointer select-none list-none flex items-center gap-1 hover:text-violet-600">
              ...
              <p className="mt-2 text-[11px] italic leading-relaxed text-sleek-text-muted border-l-2 border-violet-100 pl-3">
        ...
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] text-sleek-text-muted mt-0.5 leading-none truncate">
              Joseph Benson, 1818 · tradição wesleyana · referências clicáveis
            </p>
          </div>
        </div>
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-8 py-6">
          {loading && (
            <div className="flex flex-col items-center justify-center py-28 gap-3">
              ...
              <p className="text-[12px] text-sleek-text-muted">Carregando e traduzindo comentário de Benson…</p>
            </div>
          )}
          {!loading && error && (
          ...
          {!loading && !error && notes.length === 0 && (
            <div className="flex flex-col items-center justify-center py-28 gap-3 text-center">
              ...
              <p className="text-[13px] text-sleek-text-muted">
                Sem notas de Benson para {bookName} {chapter}.
              </p>
